...
add_mit_algorithm_layout(subsystem, cfg);
...
function add_mit_algorithm_layout(subsystem, cfg)
add_input_adapter(subsystem, cfg);
add_error_calculation(subsystem, cfg);
add_impedance_torque(subsystem, cfg);
add_current_command_limit(subsystem, cfg);
add_output_adapter(subsystem, cfg);

add_block('simulink/Sinks/Terminator', ...
    [subsystem '/diagnostic_saturated_flag_unused'], ...
    'Position', [1015 340 1045 370]);

add_line(subsystem, 'mit_in/1', 'InputAdapter/1', 'autorouting', 'on');
add_line(subsystem, 'InputAdapter/1', 'ErrorCalculation/1', 'autorouting', 'on');
add_line(subsystem, 'InputAdapter/2', 'ErrorCalculation/2', 'autorouting', 'on');
add_line(subsystem, 'InputAdapter/3', 'ErrorCalculation/3', 'autorouting', 'on');
add_line(subsystem, 'InputAdapter/4', 'ErrorCalculation/4', 'autorouting', 'on');
add_line(subsystem, 'ErrorCalculation/1', 'ImpedanceTorque/1', 'autorouting', 'on');
add_line(subsystem, 'ErrorCalculation/2', 'ImpedanceTorque/2', 'autorouting', 'on');
add_line(subsystem, 'InputAdapter/5', 'ImpedanceTorque/3', 'autorouting', 'on');
add_line(subsystem, 'InputAdapter/6', 'ImpedanceTorque/4', 'autorouting', 'on');
add_line(subsystem, 'InputAdapter/7', 'ImpedanceTorque/5', 'autorouting', 'on');
add_line(subsystem, 'ImpedanceTorque/1', 'CurrentCommandLimit/1', 'autorouting', 'on');
add_line(subsystem, 'InputAdapter/8', 'CurrentCommandLimit/2', 'autorouting', 'on');
add_line(subsystem, 'InputAdapter/9', 'CurrentCommandLimit/3', 'autorouting', 'on');
add_line(subsystem, 'CurrentCommandLimit/1', 'OutputAdapter/1', 'autorouting', 'on');
add_line(subsystem, 'CurrentCommandLimit/2', ...
    'diagnostic_saturated_flag_unused/1', 'autorouting', 'on');
add_line(subsystem, 'OutputAdapter/1', 'mit_out/1', 'autorouting', 'on');
end

function add_input_adapter(parent, cfg)
subsystem = [parent '/InputAdapter'];
add_clean_subsystem(subsystem, [115 80 250 360]);
add_block('simulink/Sources/In1', [subsystem '/mit_in'], ...
    'Position', [35 185 65 205], ...
    'OutDataTypeStr', ['Bus: ' cfg.inputBusName], ...
    'BusOutputAsStruct', 'on');
add_block('simulink/Signal Routing/Bus Selector', [subsystem '/input_selector'], ...
    'Position', [115 35 135 405], ...
...
for i = 1:size(output_specs, 1)
    y = 30 + (i - 1) * 45;
    add_block('simulink/Sinks/Out1', [subsystem '/' output_specs{i, 1}], ...
        'Position', [220 y 250 y + 20], ...
        'OutDataTypeStr', output_specs{i, 2});
    name_line(add_line(subsystem, ['input_selector/' num2str(i)], ...
        [output_specs{i, 1} '/1'], 'autorouting', 'on'), output_specs{i, 1});
end

add_line(subsystem, 'mit_in/1', 'input_selector/1', 'autorouting', 'on');
end

function add_error_calculation(parent, cfg)
subsystem = [parent '/ErrorCalculation'];
add_clean_subsystem(subsystem, [315 85 485 245]);

add_typed_inport(subsystem, 'pos_target_rad', [35 40 65 60], cfg.angleTypeName);
add_typed_inport(subsystem, 'pos_feedback_rad', [35 95 65 115], cfg.angleTypeName);
add_typed_inport(subsystem, 'vel_target_rad_s', [35 170 65 190], cfg.speedTypeName);
add_typed_inport(subsystem, 'vel_feedback_rad_s', [35 225 65 245], cfg.speedTypeName);
add_block('simulink/Sinks/Out1', [subsystem '/position_error_rad'], ...
    'Position', [455 65 485 85], ...
    'OutDataTypeStr', cfg.angleTypeName);
add_block('simulink/Sinks/Out1', [subsystem '/speed_error_rad_s'], ...
    'Position', [455 200 485 220], ...
    'OutDataTypeStr', cfg.speedTypeName);

add_block('simulink/Math Operations/Sum', [subsystem '/position_error_raw'], ...
    'Position', [110 55 140 100], ...
    'Inputs', '+-', ...
    'AccumDataTypeStr', cfg.angleTypeName, ...
    'OutDataTypeStr', cfg.angleTypeName);
add_wrap_pi_subsystem(subsystem, cfg);
add_block('simulink/Math Operations/Sum', [subsystem '/speed_error'], ...
    'Position', [165 185 195 230], ...
    'Inputs', '+-', ...
    'AccumDataTypeStr', cfg.speedTypeName, ...
    'OutDataTypeStr', cfg.speedTypeName);

add_line(subsystem, 'pos_target_rad/1', 'position_error_raw/1', ...
    'autorouting', 'on');
add_line(subsystem, 'pos_feedback_rad/1', 'position_error_raw/2', ...
    'autorouting', 'on');
add_line(subsystem, 'position_error_raw/1', 'WrapPi/1', 'autorouting', 'on');
add_line(subsystem, 'WrapPi/1', 'position_error_rad/1', 'autorouting', 'on');
add_line(subsystem, 'vel_target_rad_s/1', 'speed_error/1', ...
    'autorouting', 'on');
add_line(subsystem, 'vel_feedback_rad_s/1', 'speed_error/2', ...
    'autorouting', 'on');
add_line(subsystem, 'speed_error/1', 'speed_error_rad_s/1', ...
    'autorouting', 'on');
end

function add_wrap_pi_subsystem(parent, cfg)
subsystem = [parent '/WrapPi'];
add_clean_subsystem(subsystem, [215 45 385 110]);
add_typed_inport(subsystem, 'angle_raw_rad', [35 95 65 115], cfg.angleTypeName);
add_block('simulink/Sinks/Out1', [subsystem '/angle_wrapped_rad'], ...
    'Position', [575 95 605 115], ...
    'OutDataTypeStr', cfg.angleTypeName);

add_typed_constant(subsystem, 'two_pi', [115 150 175 175], ...
    'GJMitTwoPi', cfg.angleTypeName, '-1');
add_typed_constant(subsystem, 'neg_pi', [300 205 360 230], ...
    '-GJMitPi', cfg.angleTypeName, '-1');
add_block('simulink/Math Operations/Sum', [subsystem '/minus_2pi'], ...
    'Position', [215 50 245 95], ...
    'Inputs', '+-', ...
    'AccumDataTypeStr', cfg.angleTypeName, ...
    'OutDataTypeStr', cfg.angleTypeName);
add_block('simulink/Math Operations/Sum', [subsystem '/plus_2pi'], ...
    'Position', [215 125 245 170], ...
    'Inputs', '++', ...
    'AccumDataTypeStr', cfg.angleTypeName, ...
    'OutDataTypeStr', cfg.angleTypeName);
add_block('simulink/Signal Routing/Switch', [subsystem '/wrap_high_switch'], ...
    'Position', [320 65 370 115], ...
    'Criteria', 'u2 > Threshold', ...
    'Threshold', 'GJMitPi', ...
    'OutDataTypeStr', cfg.angleTypeName);
add_block('simulink/Logic and Bit Operations/Relational Operator', ...
    [subsystem '/is_below_neg_pi'], ...
    'Position', [400 185 450 215], ...
    'Operator', '<');
add_block('simulink/Signal Routing/Switch', [subsystem '/wrap_low_switch'], ...
    'Position', [485 80 535 130], ...
    'Criteria', 'u2 ~= 0', ...
    'OutDataTypeStr', cfg.angleTypeName);

add_line(subsystem, 'angle_raw_rad/1', 'minus_2pi/1', 'autorouting', 'on');
add_line(subsystem, 'two_pi/1', 'minus_2pi/2', 'autorouting', 'on');
add_line(subsystem, 'angle_raw_rad/1', 'plus_2pi/1', 'autorouting', 'on');
add_line(subsystem, 'two_pi/1', 'plus_2pi/2', 'autorouting', 'on');
add_line(subsystem, 'minus_2pi/1', 'wrap_high_switch/1', ...
    'autorouting', 'on');
add_line(subsystem, 'angle_raw_rad/1', 'wrap_high_switch/2', ...
    'autorouting', 'on');
add_line(subsystem, 'angle_raw_rad/1', 'wrap_high_switch/3', ...
    'autorouting', 'on');
add_line(subsystem, 'angle_raw_rad/1', 'is_below_neg_pi/1', ...
    'autorouting', 'on');
add_line(subsystem, 'neg_pi/1', 'is_below_neg_pi/2', 'autorouting', 'on');
add_line(subsystem, 'plus_2pi/1', 'wrap_low_switch/1', 'autorouting', 'on');
add_line(subsystem, 'is_below_neg_pi/1', 'wrap_low_switch/2', ...
    'autorouting', 'on');
add_line(subsystem, 'wrap_high_switch/1', 'wrap_low_switch/3', ...
    'autorouting', 'on');
add_line(subsystem, 'wrap_low_switch/1', 'angle_wrapped_rad/1', ...
    'autorouting', 'on');
end

function add_impedance_torque(parent, cfg)
subsystem = [parent '/ImpedanceTorque'];
add_clean_subsystem(subsystem, [555 90 735 280]);

add_typed_inport(subsystem, 'position_error_rad', [35 40 65 60], ...
    cfg.angleTypeName);
add_typed_inport(subsystem, 'speed_error_rad_s', [35 110 65 130], ...
    cfg.speedTypeName);
add_typed_inport(subsystem, 'ff_torque_nm', [35 250 65 270], ...
    cfg.torqueTypeName);
add_typed_inport(subsystem, 'kp_nm_per_rad', [35 75 65 95], ...
    cfg.gainTypeName);
add_typed_inport(subsystem, 'kd_nm_s_per_rad', [35 145 65 165], ...
    cfg.gainTypeName);
add_block('simulink/Sinks/Out1', [subsystem '/torque_cmd_nm'], ...
    'Position', [415 135 445 155], ...
    'OutDataTypeStr', cfg.torqueTypeName);

add_block('simulink/Math Operations/Product', [subsystem '/stiffness_torque'], ...
    'Position', [140 45 180 85], ...
    'OutDataTypeStr', cfg.torqueTypeName);
add_block('simulink/Math Operations/Product', [subsystem '/damping_torque'], ...
    'Position', [140 120 180 160], ...
    'OutDataTypeStr', cfg.torqueTypeName);
add_block('simulink/Math Operations/Sum', [subsystem '/sum_torque'], ...
    'Position', [285 105 315 170], ...
    'Inputs', '+++', ...
    'AccumDataTypeStr', cfg.torqueTypeName, ...
    'OutDataTypeStr', cfg.torqueTypeName);

add_line(subsystem, 'position_error_rad/1', 'stiffness_torque/1', ...
    'autorouting', 'on');
add_line(subsystem, 'kp_nm_per_rad/1', 'stiffness_torque/2', ...
    'autorouting', 'on');
add_line(subsystem, 'speed_error_rad_s/1', 'damping_torque/1', ...
    'autorouting', 'on');
add_line(subsystem, 'kd_nm_s_per_rad/1', 'damping_torque/2', ...
    'autorouting', 'on');
add_line(subsystem, 'stiffness_torque/1', 'sum_torque/1', ...
    'autorouting', 'on');
add_line(subsystem, 'damping_torque/1', 'sum_torque/2', 'autorouting', 'on');
add_line(subsystem, 'ff_torque_nm/1', 'sum_torque/3', 'autorouting', 'on');
add_line(subsystem, 'sum_torque/1', 'torque_cmd_nm/1', 'autorouting', 'on');
end

function add_current_command_limit(parent, cfg)
subsystem = [parent '/CurrentCommandLimit'];
add_clean_subsystem(subsystem, [800 95 970 295]);

add_typed_inport(subsystem, 'torque_cmd_nm', [35 60 65 80], ...
    cfg.torqueTypeName);
add_typed_inport(subsystem, 'torque_to_iq_gain_a_per_nm', ...
    [35 115 65 135], cfg.gainTypeName);
add_typed_inport(subsystem, 'iq_limit_a', [35 220 65 240], ...
    cfg.currentTypeName);
add_block('simulink/Sinks/Out1', [subsystem '/iq_ref_a'], ...
    'Position', [540 90 570 110], ...
    'OutDataTypeStr', cfg.currentTypeName);
add_block('simulink/Sinks/Out1', [subsystem '/saturated_flag'], ...
    'Position', [540 260 570 280], ...
    'OutDataTypeStr', cfg.floatTypeName);

add_block('simulink/Math Operations/Product', [subsystem '/iq_unsat_a'], ...
    'Position', [125 75 165 115], ...
    'OutDataTypeStr', cfg.currentTypeName);
add_block('simulink/Math Operations/Abs', [subsystem '/iq_limit_abs'], ...
    'Position', [125 215 165 245], ...
    'OutDataTypeStr', cfg.currentTypeName);
add_block('simulink/Math Operations/Gain', [subsystem '/neg_iq_limit'], ...
    'Position', [215 215 275 245], ...
    'Gain', '-1', ...
    'ParamDataTypeStr', cfg.floatTypeName, ...
    'OutDataTypeStr', cfg.currentTypeName);
add_minmax(subsystem, 'iq_upper_limited', [270 80 330 115], 'min');
add_minmax(subsystem, 'iq_limited_a', [380 80 440 115], 'max');
add_block('simulink/Math Operations/Abs', [subsystem '/iq_unsat_abs'], ...
    'Position', [270 285 310 315], ...
    'OutDataTypeStr', cfg.currentTypeName);
add_block('simulink/Logic and Bit Operations/Relational Operator', ...
    [subsystem '/is_saturated'], ...
    'Position', [360 280 410 310], ...
    'Operator', '>');
add_block('simulink/Signal Attributes/Data Type Conversion', ...
    [subsystem '/saturated_flag_float'], ...
    'Position', [450 280 500 310], ...
    'OutDataTypeStr', cfg.floatTypeName);

add_line(subsystem, 'torque_cmd_nm/1', 'iq_unsat_a/1', 'autorouting', 'on');
add_line(subsystem, 'torque_to_iq_gain_a_per_nm/1', 'iq_unsat_a/2', ...
    'autorouting', 'on');
add_line(subsystem, 'iq_limit_a/1', 'iq_limit_abs/1', 'autorouting', 'on');
...
add_line(subsystem, 'iq_limit_abs/1', 'iq_upper_limited/2', ...
    'autorouting', 'on');
add_line(subsystem, 'iq_upper_limited/1', 'iq_limited_a/1', ...
    'autorouting', 'on');
add_line(subsystem, 'neg_iq_limit/1', 'iq_limited_a/2', ...
    'autorouting', 'on');
add_line(subsystem, 'iq_limited_a/1', 'iq_ref_a/1', 'autorouting', 'on');
...
add_line(subsystem, 'is_saturated/1', 'saturated_flag_float/1', ...
    'autorouting', 'on');
add_line(subsystem, 'saturated_flag_float/1', 'saturated_flag/1', ...
    'autorouting', 'on');
end

function add_output_adapter(parent, cfg)
subsystem = [parent '/OutputAdapter'];
add_clean_subsystem(subsystem, [1070 145 1210 240]);
add_typed_inport(subsystem, 'iq_ref_a', [35 50 65 70], cfg.currentTypeName);
add_block('simulink/Signal Routing/Bus Creator', ...
    [subsystem '/mit_output_bus_creator'], ...
    'Position', [130 40 145 85], ...
    'Inputs', '1', ...
    'UseBusObject', 'on', ...
    'BusObject', cfg.outputBusName, ...
    'NonVirtualBus', 'on');
add_block('simulink/Sinks/Out1', [subsystem '/mit_out'], ...
    'Position', [215 50 245 70], ...
    'OutDataTypeStr', ['Bus: ' cfg.outputBusName]);

name_line(add_line(subsystem, 'iq_ref_a/1', 'mit_output_bus_creator/1', ...
    'autorouting', 'on'), 'iq_ref_a');
add_line(subsystem, 'mit_output_bus_creator/1', 'mit_out/1', ...
    'autorouting', 'on');
end
...
function add_typed_inport(model, block_name, position, data_type)
add_block('simulink/Sources/In1', [model '/' block_name], ...
    'Position', position, ...
    'OutDataTypeStr', data_type);
end

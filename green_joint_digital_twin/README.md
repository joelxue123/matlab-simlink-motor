...
不要把当前固件的 MIT `kp/kd` 直接理解为输出端 `Nm/rad`、`Nm*s/rad`。如果未来要做正规的物理阻抗接口，应把协议层阻抗先转换成电流，或者新增 MBD impedance module 明确单位合同。
...
结论：当前默认 `Kd=0.1` 对 1615 新辨识惯量偏欠阻尼，但没有触发 4A 电流饱和。
按 1615 现有 4A 电流限制，`15Hz/zeta=1` 是更稳的主线候选；`20Hz/zeta=1`
在 0.2rad 阶跃已经会撞 4A 限幅，更适合小角度或提高限流后的评估。
